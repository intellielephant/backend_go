...
const (
	...
	version         = "v1"
	yidun_secretId  = "31c685f4d7021db08b6991434c616076" //产品密钥ID，产品标识
	yidun_secretKey = "5e2f5ef7ec43aafebb8b6e857953dc58" //产品私有密钥，服务端生成签名信息使用，请严格保管，避免泄露
	businessId      = "27e1a55f23b644528dc1352fa552d5b0" //业务ID，易盾根据产品业务特点分配
)

// 请求易盾接口
func check(params url.Values) *simplejson.Json {
	params["secretId"] = []string{yidun_secretId}
...
func gen_signature(params url.Values) string {
	...
	paramStr += yidun_secretKey
	md5Reader := md5.New()
	md5Reader.Write([]byte(paramStr))
	return hex.EncodeToString(md5Reader.Sum(nil))
}

func OneClick(token, accessToken string) (bool, string) {
	params := url.Values{
		//token为易盾返回的token
		"token": []string{token},
